...
namespace NMemoryCache.Benchmarks
{
    ...
    public class CacheBenchmark
    {
        ...
        public void Run(int keys, int tags, int tagsPerKey, int lifetimeSec, int invalidationMsec)
        {
            ...
            while (true)
            {
                ...
                long opsPerSec = Interlocked.Exchange(ref _currentCount, 0) / (sw.ElapsedMilliseconds - lastElapsed);
        ...
        private void CachingThread()
        {
            ...
            if (_tags.Length == 0)
            {
                while (true)
                {
                    ...
                    Interlocked.Add(ref _currentCount, OperationStep);
                }
            }
